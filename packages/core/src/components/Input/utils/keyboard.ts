 ...
 * 创建键盘事件处理器
 */
export function createKeyboardHandler(shortcuts: Record<string, KeyboardShortcut>) {
  return (event: KeyboardEvent, callback: (shortcut: string, event: KeyboardEvent) => void) => {
 ...
 * 创建平台相关的快捷键
 */
export function createPlatformShortcut(key: string, useModifier = true): KeyboardShortcut {
...
export class InputKeyboardHandler {
   ...
   * 添加快捷键
   */
  addShortcut(name: string, shortcut: KeyboardShortcut, handler: (event: KeyboardEvent) => void) {
   ...
   * 设置提交快捷键
   */
  setSubmitShortcut(type: 'enter' | 'shiftEnter' | 'ctrlEnter' | 'altEnter' | 'metaEnter') {
    // 移除现有的提交快捷键
    ...
    switch (type) {
      case 'enter':
        ...
        break
      case 'shiftEnter':
        this.shortcuts.submitWithShift = { key: 'Enter', shift: true, preventDefault: true }
        break
      case 'ctrlEnter':
        this.shortcuts.submitWithCtrl = { key: 'Enter', ctrl: true, preventDefault: true }
        break
      case 'altEnter':
        this.shortcuts.submitWithAlt = { key: 'Enter', alt: true, preventDefault: true }
        break
      case 'metaEnter':
        this.shortcuts.submitWithMeta = { key: 'Enter', meta: true, preventDefault: true }
        break
    }
  }
}
