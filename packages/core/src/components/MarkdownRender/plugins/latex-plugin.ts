 ...
 * 渲染内联 LaTeX 公式
 */
function renderInlineLatex(latex: string, options: LatexPluginOptions = {}): string {
  try {
 ...
 * 渲染块级 LaTeX 公式
 */
function renderBlockLatex(latex: string, options: LatexPluginOptions = {}): string {
  try {
    return katex.renderToString(latex, {
      displayMode: true,
      strict: options.strict || 'warn',
 ...
 */
export const latexPlugin: PluginSimple = (md, options: LatexPluginOptions = {}) => {
  // 内联 LaTeX 公式：$...$
  ...
  // 块级 LaTeX 公式：$$...$$
  const blockLatexRule = (state: any, start: number, end: number, silent: boolean) => {
