...
export class PluginManager {
   ...
   * 根据名称列表获取启用的插件
   */
  private getEnabledPluginsByName(enabledNames: string[]): PluginRegistration[] {
    return Array.from(this.plugins.values())
      .filter(plugin => plugin.enabled === true && enabledNames.includes(plugin.name))
  }
...
// 导出便捷方法
export const registerPlugin = (registration: PluginRegistration) => pluginManager.register(registration)
export const unregisterPlugin = (name: string) => pluginManager.unregister(name)
